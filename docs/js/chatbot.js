...
function toggleChat() {
  const chat = dolet chatHistory = JSON.parse(localStorage.getItem("chatHistory") || "[]");
...
function toggleChat() {
  ...
  if (!isOpen) {
    ...
    }, 0);let chatHistory = JSON.parse(localStorage.getItem("chatHistory") || "[]");
...
}cument.getElementById("chat-container");
  const isOpen = chat.style.display === "flex";
